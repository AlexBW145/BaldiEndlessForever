...
class MiscPatches
{
    ...
    [HarmonyPatch(typeof(Credits), "Start"), HarmonyPrefix]
    static void CreditsForever(Credits __instance)
    {
        var credits = GameObject.Instantiate(__instance.screens[0], null);
        credits.GetComponentsInChildren<TextMeshProUGUI>()[0].text = @"Infinite Floors Forever
...
Fun House - Ripped from Marcello's Fun House(?)
        ...
        GameObject.Destroy(credits.GetComponentsInChildren<TextMeshProUGUI>()[1]);
        GameObject.Destroy(credits2.GetComponentsInChildren<TextMeshProUGUI>()[1]);
        credits.gameObject.SetActive(false);
        credits2.gameObject.SetActive(false);
        __instance.screens.InsertRange(7, [credits, credits2]);
    }

    /*[HarmonyPatch(typeof(Credits), "Update"), HarmonyPrefix]
